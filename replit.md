...
- **Internationalization**: Full Korean/English i18n support via `LanguageContext` and `lib/i18n.ts`. All UI strings across menus, modals, and components use `t("section", "key")` translation calls.
  - **i18n 검증 (3단계)**: (1) `translations` 객체에 `satisfies Record<string, Record<string, Record<LanguageCode, string>>>`가 적용되어 한 언어만 추가하면 컴파일 에러. (2) `tests/i18n-completeness.test.ts`/`tests/i18n-meta.test.ts`가 모든 leaf의 키 셋이 `SUPPORTED_LANGUAGES`와 정확히 동일하고 값이 비어있지 않음을 확인. (3) `npx tsx scripts/check-i18n-keys.ts`가 코드에서 호출하는 정적 `t("ns","key")`가 모두 정의돼 있는지 검사하고, 누락 시 exit 1. 런타임에는 `createT`가 dev 모드에서 누락 키/언어 폴백을 `console.warn` + Sentry breadcrumb으로 보고한다.
  - **새 언어 추가 절차**: (a) `lib/i18n.ts` 상단의 `SUPPORTED_LANGUAGES` 배열에 코드 추가(예: `"ja"`). (b) `tsc`가 모든 leaf에서 누락 키를 알려주면 한 곳씩 채운다. (c) `tests/i18n-meta.test.ts`로 키 셋 동치성 회귀 가드. (d) 폴백은 `FALLBACK_LANGUAGE`(현재 `en`) → 키 자체 순. 디바이스 로케일 자동 감지/수동 선택은 `LanguageContext`가 `isLanguageCode`로 검증하므로 추가 분기 불필요.
