...
This project is a metronome application built with Expo/React Native and a small Express backend. In production, the mobile or web client is the primary user-facing app, while the Express server serves the landing page and static web assets.
...
- **Imported and recorded audio files** -- user-controlled media parsed locally on the client device.
...
- **Client to server** -- the mobile/web app sends ordinary application requests to the Express server. The server must treat all body fields, headers, and content types as attacker-controlled.
...
- **Public surfaces:** `/`, `/manifest`, static asset serving, `/practice`, and custom-scheme deep links.
