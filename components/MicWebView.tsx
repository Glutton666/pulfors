...
var running=false,audioCtx,analyser,source,stream,raf;
...
async function startMic(){if(running)return;running=true;try{stream=await navigator.mediaDevices.getUserMedia({audio:{echoCancellation:false,noiseSuppression:false,autoGainControl:false}});audioCtx=new(window.AudioContext||window.webkitAudioContext)();analyser=audioCtx.createAnalyser();analyser.fftSize=4096;source=audioCtx.createMediaStreamSource(stream);source.connect(analyser);var buf=new Float32Array(analyser.fftSize);function detect(){if(!running)return;analyser.getFloatTimeDomainData(buf);autoCorr(buf,audioCtx.sampleRate);raf=requestAnimationFrame(detect);}detect();}catch(e){window.ReactNativeWebView.postMessage(JSON.stringify({type:"freq",frequency:null,note:null}));}}
