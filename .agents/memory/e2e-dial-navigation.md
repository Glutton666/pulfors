...
**How to apply:**
- Recipe: click `mode-cycle-label` → fan opens at top-center anchor `(winW/2, 0)`, `centAng=90°`, `ANGLE_STEP=34°`, `ICON_R=104`, MODES=[beat,bar,score,note,practice,stage,menu]. Click at `(winW/2 + cos(90+off·34°)·104, sin·104)` to select (tap radius 52px), then click the overlay (e.g. `(winW/2, h·0.7)`) to confirm.
- From beat: score = offset +2 → `(cx−96, 39)`; menu = offset −1 → `(cx+58, 86)`. From menu: beat = +1 → `(cx−58, 86)`.
