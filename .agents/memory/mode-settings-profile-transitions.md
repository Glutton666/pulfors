...
**How to apply:** Any independently persisted UI/audio profile switch must keep profile storage separate from transient rendered state and include an A→B→A regression test. Keep Bar rhythm fields out of generic profile hydration and persist Bar BPM through its own update path.