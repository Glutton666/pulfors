...
**How to apply:** When a feature's only entry point is a chained gesture sequence (swipe + long-press, drag + drop, etc.), don't burn more than 2 e2e attempts on it. Split the gestures into focused component tests when possible. In particular, an empty Bar session requires a PanResponder swipe to create its first row, so a drawer-to-row drag e2e otherwise tests two gesture recognizers before reaching the target behavior. Fall back to `tsc --noEmit`, targeted unit tests, and a manual trace of prop/state wiring. Document this as a known e2e limitation rather than as an unverified product risk.
